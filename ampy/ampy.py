...
# Edits by <jbaldwin8889>
# Added "ListOf" type to allow passing of lists
# See SUBLICENSE for licensing details
...
class ListOf(Argument):
    ...
    def fromString(self,bytes):
        retlist = []
        while len(bytes):
            valuelen = struct.unpack("!H",bytes[:2])[0]
            bytes = bytes[2:]
            retlist.append(self.argtype.fromString(bytes[:valuelen]))
    ...
    def toString(self,obj):
        retstring = ""
        for value in obj:
            elementstring = self.argtype.toString(value)
            retstring += struct.pack("!H",len(elementstring))
            retstring += elementstring
        return retstring
